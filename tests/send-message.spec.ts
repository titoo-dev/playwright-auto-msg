  ...
  try {
    ...
    for (const user of users) {
      if (!user.userUrl || !user.message) {
        throw new Error(
          'Chaque objet doit avoir les propriétés "userUrl" et "message"'
        );
      }
    }

    console.log(
      `📄 ${users.length} utilisateur(s) chargé(s) depuis ${jsonFilePath}`
    );
    return users;
  } catch (error) {
    console.error(
...
async function performLogin(
  ...
  try {
    ...
    await page.getByTestId("royal-email").fill(credentials.email);
    console.log("📧 Email saisi");

    await page.getByTestId("royal-pass").click();
    await page.getByTestId("royal-pass").fill(credentials.password);
    ...
    if (loginSuccessful) {
      ...
      const errorMessage = await page
        .locator('[data-testid="royal_login_form"] [role="alert"]')
        .textContent()
        .catch(() => null);
...
async function sendQuickMessageIfAlreadyLoggedIn(
  page: Page,
  message: string = "test 49 lines"
): Promise<void> {
...
test("Ouvrir Facebook avec profil existant et connexion auto", async () => {
  ...
  console.log(
    `🚀 Début de l'automatisation pour ${users.length} utilisateur(s)`
  );
